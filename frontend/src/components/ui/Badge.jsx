export function Badge({ status }) {
  const map = {
    ...
    vacante:   "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300",
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${map[status] || "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300"}`}>
      {status}
    </span>
  );
}